...
class FicharioAPI implements FicharioAPIType {
    ...
    get isAdmin() {
        return this.profile === "CompanyOwner" || this.profile === "CompanyManager";
    }
    
    constructor({ token = "", company = "", profile = "" } = {}) {
    ...
    static async new(data: AuthType): Promise<FicharioAPI> {
        ...
        if(ficharioAPI.profile === "CompanyOwner" || ficharioAPI.profile === "CompanyManager") {
            try {
        ...
        // console.log(`Connection with fichar.io established!`)
    ...
    async request(options: RequestOptionsType): Promise<unknown> {
        ...
        try {
            ...
            // console.log(`Making request to ${options.method} ${url}`)
            // if (options.data) console.log(`With data: ${JSON.stringify(options.data)}`)
    ...
    async makeRequest(config: any, retries = 6): Promise<AxiosResponse<any, any>> {
        try {
            ...
            if (retries > 0 && error.response && (error.response.status === 502 || error.response.status === 429)) {
                ...
                if (error.response) {
                    console.error(`Request error "${config?.path}": ${error.response.status} - ${error.response.statusText}`);
                ...
                throw {
                    error: "Request to Fichario failed",
                }
            }
        }
    }
    ...
    async getDevices({ admin = true } = {}): Promise<Array<DeviceType>> {
        const endpoint = admin && this.isAdmin ?
    ...
    async getDeviceInfos({ deviceID, admin = true }: {deviceID: string, admin?: boolean}): Promise<Array<DeviceInfoType>> {
        ...
        let endpoint = admin && this.isAdmin ?
